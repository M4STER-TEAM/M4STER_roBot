...
function chat_new_user(msg)
   ...
   if not message then
      return 'Hola '..name..', bienvenido a '..chat..'!'
   end
...
local function run(msg, matches)
   ...
   elseif matches[1] == 'setwelcome' then
      ...
      return 'La nueva bienvenida es:\n\n'..matches[2]
   elseif matches[1] == 'getwelcome' then
      print(msg.to.id)
      local hash = 'welcome:'..msg.to.id
      local wel = redis:get(hash)
      if not wel then
         return 'No hay bienvenid fijada'
      end
      return wel
   ...
   if not permissions(msg.from.id, msg.to.id, "welcome") then
      ...
      local wel = redis:get(hash)
      if not wel then
         return 'No hay despedida fijada'
      end
      return wel
